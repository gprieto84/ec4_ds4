...
db_passwd = os.environ.get('DB_USER_PASSWORD')
db_user = os.environ.get('DB_USER')
db_host = os.environ.get('DB_HOST')

engine = create_engine(f'postgresql://{db_user}:{db_passwd}@{db_host}/nps_demo_db')
df = pd.read_sql("SELECT * from trades", engine.connect(), parse_dates=('Entry time',))

df['YearMonth'] = df['Entry time'].values.astype('datetime64[M]')

app = dash.Dash(__name__, external_stylesheets=['https://codepen.io/uditagarwal/pen/oNvwKNP.css', 'https://codepen.io/uditagarwal/pen/YzKbqyV.css'])

#Helper function
def filter_df(df,exchange, leverage, start_date, end_date):
    dff = df[(df['Exchange'] == exchange) & (df['Margin'] == int(leverage)) & (df['Entry time'] >= start_date) & (df['Entry time'] <= end_date)]
    return dff
...
def update_daily_btc(exchange, leverage, start_date, end_date):
    ...
    trace = go.Scatter(x=dff["Entry time"], y=dff['BTC Price'], name='Daily BTC price', mode='lines',
                                marker={'size': 8, "opacity": 0.6, "line": {'width': 0.5}}, )
    return{
        'data':[trace],
        "layout": go.Layout(title="Daily BTC")
    }
...
def update_balance(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    trace = go.Scatter(x=dff["Entry time"], y=dff['Entry balance'], name='Balance overtime', mode='lines',
                                marker={'size': 8, "opacity": 0.6, "line": {'width': 0.5}}, )
    return{
        'data':[trace],
...
def update_barchart(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    df_s = dff[dff['Trade type']=='Short']
    df_l = dff[dff['Trade type']=='Long']
    trace1 = go.Bar(x=df_s['Entry time'], y=df_s['Pnl (incl fees)'], name='Short')
    trace2 = go.Bar(x=df_l['Entry time'], y=df_l['Pnl (incl fees)'], name='Long')
    return {
        'data': [trace1,trace2]
    }
